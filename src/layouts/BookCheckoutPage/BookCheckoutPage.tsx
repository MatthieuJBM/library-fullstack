...
export const BookCheckoutPage = () => {
  ...
  useEffect(() => {
    const fetchBookReviews = async () => {
      ...
      if (!responseReviews.ok) {
        throw new Error("Something went wrong fetching the reviews.");
      }
  ...
  useEffect(() => {
    const fetchUserReviewBook = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        if (!userReview.ok) {
          throw new Error("Something went wrong retrieving the user review.");
        }
    ...
    const fetchUserCheckedOutBook = async () => {
      if (authState && authState.isAuthenticated) {
        ...
        if (!bookCheckedOut.ok) {
          throw new Error(
            "Something went wrong checking the book if already checked out!"
          );
        }
  ...
  async function submitReview(starInput: number, reviewDescription: string) {
    ...
    if (!returnResponse.ok) {
      throw new Error("Something went wrong leaving the review!");
    }
